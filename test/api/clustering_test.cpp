...
TEST(clustering, partitioning)
{
    ...
    EXPECT_EQ(expected_partitions.size(), partitions.size());

    if (expected_partitions.size() == partitions.size())
    {
        for (size_t partition_index = 0; partition_index < expected_partitions.size(); ++partition_index)
        {
            EXPECT_EQ(expected_partitions[partition_index].size(), partitions[partition_index].size());
            if (expected_partitions[partition_index].size() == partitions[partition_index].size())
            {
...
TEST(clustering, strict_clustering)
{
    ...
    if (expected_clusters.size() == clusters.size())
    {
        ...
        {
            EXPECT_EQ(expected_clusters[cluster_index].get_cluster_size(), clusters[cluster_index].get_cluster_size()) << "Cluster " << cluster_index << " of unexpected size";

            if (expected_clusters[cluster_index].get_cluster_size() == clusters[cluster_index].get_cluster_size())
            {
